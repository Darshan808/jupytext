...
async def test_rename_inconsistent_path(cm, tmpdir):
    ...
    await cm.get("notebook_suffix.ipynb")
    with pytest.raises(HTTPError):
        await cm.rename_file("notebook_suffix.ipynb", "new.ipynb")

    assert not os.path.isfile(new_file)
    assert os.path.isfile(org_file)


async def test_pair_unpair_notebook(cm, tmpdir):
    ...
    nb2 = (await cm.get(tmp_md))["content"]
    compare_notebooks(nb, nb2)

    # unpair and save as md
    del nb["metadata"]["jupytext"]
    await cm.save(model=notebook_model(nb), path=tmp_md)
    nb2 = (await cm.get(tmp_md))["content"]
...
async def test_load_save_rename(ipynb_py_R_jl_file, cm, tmpdir):
    ...
    # rename ipynb
    await cm.rename(tmp_ipynb, "new.ipynb")
    ...
    await cm.delete("new.Rmd")
    assert not os.path.isfile(str(tmpdir.join("new.Rmd")))
    model = await cm.get("new.ipynb", content=False)
    assert "last_modified" in model
    await cm.save(model=notebook_model(nb), path="new.ipynb")
    assert os.path.isfile(str(tmpdir.join("new.Rmd")))

    await cm.delete("new.Rmd")
    await cm.rename("new.ipynb", tmp_ipynb)
...
async def test_pair_plain_script(percent_file, cm, tmpdir, caplog):
    ...
    nb2 = (await cm.get(tmp_py))["content"]
...
async def test_load_save_rename_nbpy(ipynb_py_file, cm, tmpdir):
    ...
    await cm.save(model=notebook_model(nb), path=tmp_nbpy)
    nbpy = await cm.get(tmp_nbpy)
...
async def test_load_save_py_freeze_metadata(python_file, cm, tmpdir):
    ...
    nb = (await cm.get(tmp_nbpy))["content"]
    await cm.save(model=notebook_model(nb), path=tmp_nbpy)
...
async def test_load_text_notebook(cm, tmpdir):
    cm.root_dir = str(tmpdir)
    ...
    py_model = await cm.get(nbpy, content=False)
    assert py_model["type"] == "notebook"
    assert py_model["content"] is None

    py_model = await cm.get(nbpy, content=True)
    assert py_model["type"] == "notebook"
    ...
    await cm.save(nb_model, "notebook.ipynb")
    nb_model = await cm.get("notebook.ipynb", content=True)
...
async def test_load_save_rename_notebook_with_dot(ipynb_py_file, cm, tmpdir):
    ...
    # rename py
    await cm.rename(tmp_nbpy, "2.new_notebook.py")
    assert not os.path.isfile(str(tmpdir.join(tmp_ipynb)))
    assert not os.path.isfile(str(tmpdir.join(tmp_nbpy)))
...
async def test_load_save_rename_nbpy_default_config(ipynb_py_file, cm, tmpdir):
    ...
    # open ipynb
    nbipynb = await cm.get(tmp_ipynb)
...
async def test_load_save_rename_non_ascii_path(ipynb_py_file, cm, tmpdir):
    ...
    await cm.rename(tmp_nbpy, "nêw.nb.py")
    ...
    await cm.rename("nêw.ipynb", tmp_ipynb)
...
async def test_outdated_text_notebook(python_notebook, cm, tmpdir):
    ...
    nb = python_notebook
    await cm.save(model=notebook_model(nb), path="notebook.py")
    model_py = await cm.get("notebook.py", load_alternative_format=False)
    model_ipynb = await cm.get("notebook.ipynb", load_alternative_format=False)
...
async def test_outdated_text_notebook_no_diff_ok(cm, tmpdir, python_notebook):
    # 1. write py ipynb
    cm.formats = "py,ipynb"
    cm.outdated_text_notebook_margin = 0
    cm.root_dir = str(tmpdir)
...
async def test_outdated_text_notebook_diff_is_shown(cm, tmpdir, python_notebook):
    # 1. write py ipynb
    cm.formats = "py,ipynb"
    cm.outdated_text_notebook_margin = 0
    cm.root_dir = str(tmpdir)
...
async def test_reload_notebook_after_jupytext_cli(python_notebook, cm, tmpdir):
    ...
    # test reload
    nb1 = (await cm.get("notebook.py"))["content"]
    nb2 = (await cm.get("notebook.ipynb"))["content"]
...
async def test_load_save_percent_format(percent_file, cm, tmpdir):
    ...
    # open python, save
    nb = (await cm.get(tmp_py))["content"]
...
async def test_open_using_preferred_and_default_format_174(ipynb_py_file, cm, tmpdir):
    ...
    # save to ipynb and py
    await cm.save(model=model, path="notebook.ipynb")

    assert os.path.isfile(tmp_py)
    os.remove(tmp_ipynb)

    # read py file
    model2 = await cm.get("python/notebook.py")
    compare_notebooks(model2["content"], model["content"])

    # move py file to the another folder
    shutil.move(tmp_py, tmp_py2)
    model2 = await cm.get("other/notebook.py")
    compare_notebooks(model2["content"], model["content"])
    await cm.save(model=model, path="other/notebook.py")
...
async def test_kernelspec_are_preserved(ipynb_py_file, cm, tmpdir):
    ...
    # read ipynb
    model2 = await cm.get("notebook.ipynb")
    compare_notebooks(model2["content"], model["content"])
...
async def test_save_to_light_percent_sphinx_format(ipynb_py_file, cm, tmpdir):
    ...
    model = await cm.get(path=tmp_pct_py)
    compare_notebooks(model["content"], nb)

    model = await cm.get(path=tmp_lgt_py)
    compare_notebooks(model["content"], nb)

    model = await cm.get(path=tmp_spx_py)
    # (notebooks not equal as we insert %matplotlib inline in sphinx)

    model = await cm.get(path=tmp_ipynb)
    compare_notebooks(model["content"], nb)
...
async def test_pair_notebook_with_dot(ipynb_py_file, cm, tmpdir):
    ...
    model = await cm.get(path=tmp_py)
...
async def test_preferred_formats_read_auto(cm, tmpdir):
    tmp_py = "notebook.py"
...
async def test_save_in_auto_extension_global(ipynb_py_R_jl_file, cm, tmpdir):
    ...
    model = await cm.get(path=tmp_script)
...
async def test_global_auto_pairing_works_with_empty_notebook(cm, tmpdir):
    nb = new_notebook()
    tmp_ipynb = str(tmpdir.join("notebook.ipynb"))
    tmp_py = str(tmpdir.join("notebook.py"))
    ...
    model = await cm.get(path="notebook.ipynb")
    compare_notebooks(model["content"], nb)
    ...
    nb2 = (await cm.get(path="notebook.ipynb"))["content"]
...
async def test_metadata_filter_is_effective(ipynb_py_R_jl_file, cm, tmpdir):
    ...
    # load notebook
    nb = (await cm.get(tmp_ipynb))["content"]
    ...
    nb3 = (await cm.get(tmp_script))["content"]

    compare_notebooks(nb3, nb)


async def test_no_metadata_added_to_scripts_139(cm, tmpdir):
...
"""
    ...
    # save notebook
    await cm.save(model=model, path="script.py")
...
async def test_local_format_can_deactivate_pairing(ipynb_py_file, ext, cm, tmpdir):
    ...
    # save notebook
    await cm.save(model=notebook_model(nb), path="notebook" + ext)
    ...
    nb2 = (await cm.get("notebook" + ext))["content"]
    compare_notebooks(nb2, nb)

    # resave, check again
    await cm.save(model=notebook_model(nb2), path="notebook" + ext)

    assert os.path.isfile(str(tmpdir.join("notebook.py"))) == (ext == ".py")
    assert os.path.isfile(str(tmpdir.join("notebook.ipynb"))) == (ext == ".ipynb")
    nb3 = (await cm.get("notebook" + ext))["content"]
    compare_notebooks(nb3, nb)
...
async def test_global_pairing_allows_to_save_other_file_types(rmd_file, cm, tmpdir):
    ...
    # save notebook
    await cm.save(model=notebook_model(nb), path="notebook.Rmd")
    ...
    nb2 = (await cm.get("notebook.Rmd"))["content"]
    compare_notebooks(nb2, nb)
...
async def test_python_kernel_preserves_R_files(r_file, cm, tmpdir):
    ...
    model = await cm.get("script.R")
    model["content"].metadata["kernelspec"] = kernelspec_from_language("python")
    await cm.save(model=model, path="script.R")
...
async def test_pair_notebook_in_another_folder(cm, tmpdir):
    cm.root_dir = str(tmpdir)
    ...
    await cm.get("notebooks/notebook_name.ipynb")
    await cm.get("scripts/notebook_name.py")


async def test_pair_notebook_in_dotdot_folder(cm, tmpdir):
    cm.root_dir = str(tmpdir)
...
"""
    ...
    nb = (await cm.get("notebook.md"))["content"]
...
async def test_load_then_change_formats(cm, tmpdir):
    ...
    model = await cm.get("nb.ipynb")
    assert model["content"].metadata["jupytext"]["formats"] == "ipynb,py:light"

    await cm.save(model, path="nb.ipynb")
    assert os.path.isfile(tmp_py)
...
async def test_set_then_change_formats(cm, tmpdir):
    tmp_py = str(tmpdir.join("nb.py"))
    nb = new_notebook(metadata={"jupytext": {"formats": "ipynb,py:light"}})

    cm.root_dir = str(tmpdir)

    await cm.save(model=notebook_model(nb), path="nb.ipynb")
    assert os.path.isfile(tmp_py)
    assert read(tmp_py).metadata["jupytext"]["formats"] == "ipynb,py:light"
    os.remove(tmp_py)
...
async def test_share_py_recreate_ipynb(ipynb_py_R_jl_file, cm, tmpdir):
    ...
    model_ipynb = await cm.save(model=notebook_model(nb), path="nb.ipynb")
    ...
    model = await cm.get("nb.py")
    await cm.save(model=model, path="nb.py")
...
async def test_vim_folding_markers(cm, tmpdir):
    tmp_ipynb = str(tmpdir.join("nb.ipynb"))
    tmp_py = str(tmpdir.join("nb.py"))
...
# end region"""
    ...
    nb2 = (await cm.get("nb.ipynb"))["content"]
    compare_notebooks(nb2, nb)
...
async def test_vscode_pycharm_folding_markers(cm, tmpdir):
    tmp_ipynb = str(tmpdir.join("nb.ipynb"))
    tmp_py = str(tmpdir.join("nb.py"))
...
async def test_open_file_with_cell_markers(cm, tmpdir):
    tmp_py = str(tmpdir.join("nb.py"))
...
"""
    ...
    nb = (await cm.get("nb.py"))["content"]
    assert len(nb.cells) == 1

    await cm.save(model=notebook_model(nb), path="nb.py")
...
async def test_save_file_with_cell_markers(cm, tmpdir):
    tmp_py = str(tmpdir.join("nb.py"))
...
""",
    ...
    nb2 = (await cm.get("nb.py"))["content"]
    compare_notebooks(nb2, nb)
    assert nb2.metadata["jupytext"]["cell_markers"] == "region,endregion"


async def test_notebook_extensions(cm, tmpdir, cwd_tmpdir):
    nb = new_notebook()
    write(nb, "script.py")
    ...
    model = await cm.get("notebook.Rmd")
    assert model["type"] == "notebook"
...
async def test_notebook_extensions_in_config(cm, tmpdir, cwd_tmpdir):
    nb = new_notebook()
    write(nb, "script.py")
    write(nb, "notebook.Rmd")
    write(nb, "notebook.ipynb")
...
async def test_download_file_318(cm, tmpdir):
    tmp_ipynb = str(tmpdir.join("notebook.ipynb"))
    tmp_py = str(tmpdir.join("notebook.py"))
    ...
    model = await cm.get("notebook.ipynb", content=True, type=None, format=None)
    assert model["type"] == "notebook"


async def test_markdown_and_r_extensions(cm, tmpdir):
    ...
    model = await cm.get("notebook.markdown")
    assert model["type"] == "notebook"


async def test_server_extension_issubclass():
...
async def test_multiple_pairing(cm, tmpdir):
    ...
    await cm.save(model=notebook_model(nb("saved from cm")), path="notebook.ipynb")
    ...
    model = await cm.get("notebook.md")
    compare_notebooks(model["content"], nb("md edited"))
    ...
    model_ipynb = await cm.get(
        "notebook.ipynb", content=False, load_alternative_format=False
    )
    model_md = await cm.get("notebook.md", content=False, load_alternative_format=False)
    model_py = await cm.get("notebook.py", content=False, load_alternative_format=False)
...
async def test_new_untitled(cm, tmpdir):
    cm.root_dir = str(tmpdir)

    # untitled is "Untitled" only when the locale is English #636
    untitled, ext = (await cm.new_untitled(type="notebook"))["path"].split(".")
    assert untitled
    assert ext == "ipynb"

    # Jupytext related files
    assert (await cm.new_untitled(type="notebook", ext=".md"))[
        "path"
    ] == untitled + "1.md"
    assert (await cm.new_untitled(type="notebook", ext=".py"))[
        "path"
    ] == untitled + "2.py"
    ...
    for ext in [".py", ".md"]:
        model = await cm.new_untitled(type="file", ext=ext)
        assert model["content"] is None
        assert model["path"] == f"untitled{ext}"
    assert (await cm.new_untitled(type="directory"))["path"] == "Untitled Folder"


async def test_nested_prefix(cm, tmpdir):
    cm.root_dir = str(tmpdir)
...
async def test_timestamp_is_correct_after_reload_978(python_notebook, cm, tmp_path):
    ...
    await cm.save(notebook_model(nb), path="nb.ipynb")
    assert ipynb_py_R_jl_file.exists()
    assert py_file.exists()

    # and reload to get the original timestamp
    org_model = await cm.get("nb.ipynb")
